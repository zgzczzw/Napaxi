...
impl Database {
    ...
    pub fn new(db_path: String, flags: OpenFlags) -> Database {
        ...
        LIBSQL_INIT.call_once(|| {
            ...
            // On iOS, the system pre-initializes SQLite for its own use, which
            // causes sqlite3_config() to return SQLITE_MISUSE (21). In that
            // case we try sqlite3_shutdown() first and retry. If it still fails
            // we log a warning but continue — the bundled libsql SQLite is
            // compiled with SQLITE_THREADSAFE=1 (serialized) by default, so
            // the threading mode is already correct even if we cannot
            // reconfigure it at runtime.
            unsafe {
                let mut rc = ffi::sqlite3_config(ffi::SQLITE_CONFIG_SERIALIZED);
                if rc != ffi::SQLITE_OK {
                    // Try resetting SQLite state and retrying
                    ffi::sqlite3_shutdown();
                    rc = ffi::sqlite3_config(ffi::SQLITE_CONFIG_SERIALIZED);
                }
                if rc != ffi::SQLITE_OK {
                    #[cfg(target_os = "ios")]
                    eprintln!(
                        "libsql: sqlite3_config(SERIALIZED) returned {} — \
                         continuing anyway (iOS system SQLite conflict)",
                        rc
                    );
                ...
                let init_rc = ffi::sqlite3_initialize();
                if init_rc != ffi::SQLITE_OK {
                    eprintln!("libsql: sqlite3_initialize() returned {}", init_rc);
                }
            }
        });
